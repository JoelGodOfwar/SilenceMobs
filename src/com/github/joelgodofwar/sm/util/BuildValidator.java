...
public class BuildValidator {
	...
	public boolean isBuildValid() {
		this.sm = SilenceMobs.getInstance();
        LOGGER = sm.LOGGER;
		String version = sm.getDescription().getVersion();
		this.DEV_BUILD_START_TIME = SilenceMobs.DEV_BUILD_START_TIME;
		...
		if ((versionParts.length > 3) && versionParts[3].startsWith("D")) {
			...
			LOGGER.debug("Dev-build %s valid, expires on %s.", version, new Date(expirationTime));
		}
		return true;
	}
	...
	public void sendWarnings() {
		...
		if (daysRemaining >= EXPIRATION_DAYS) {
			LOGGER.log("[" + SilenceMobs.THIS_NAME + "] Dev-build valid for " + EXPIRATION_DAYS + " days");
		} else if (daysRemaining <= 7) {
			LOGGER.warn("[" + SilenceMobs.THIS_NAME + "] Dev-build expires in " + daysRemaining + " day(s)!");
		} else if (daysRemaining % 7 == 0) {
			LOGGER.warn("[" + SilenceMobs.THIS_NAME + "] Dev-build expires in " + daysRemaining + " days");
		}
	}
}